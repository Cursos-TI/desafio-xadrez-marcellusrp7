...
// Definição de variáveis constantes para os limites de movimentação (Requisito do Desafio) [cite: 56, 72]
const int PASSOS_TORRE = 5;   // Torre move-se 5 casas [cite: 49, 87]
const int PASSOS_BISPO = 5;   // Bispo move-se 5 casas [cite: 48, 87]
const int PASSOS_RAINHA = 8;  // Rainha move-se 8 casas [cite: 50, 87]
...
int main() {
    ...
    for (int vertical = 1, horizontal = 1; vertical <= 3; vertical++) {
        ...
        // Loop aninhado interno para o movimento horizontal (1 casa para a direita) [cite: 83, 85, 87]
    ...
    return 0; // Retorno padrão da função main executada com sucesso [cite: 7]
}
